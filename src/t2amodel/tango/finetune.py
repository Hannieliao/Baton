...
def parse_args():
    ...
    parser.add_argument(
        "--finetune_file", type=str, default="data/2label_Integrity_HDRD.json",
        help="A csv or a json file containing the fine-tuning data."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--feedback_column", type=str, default=None,
        help="The name of the column in the datasets containing the human labeled or modeled labeled values.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--per_device_finetune_batch_size", type=int, default=2,
        help="Batch size (per device) for the training dataloader.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--learning_rate", type=float, default=1e-5, # Fine-tuning with smaller lr
...
class Text2AudioDataset(Dataset):
    def __init__(self, dataset, prefix, text_column, audio_column, num_examples=-1, feedback_column=None):
        ...
        # Add feedback column
        if feedback_column:
            self.scores = [float(score) for score in dataset[feedback_column]]
        else:
            self.scores = [None] * len(self.inputs)

        self.mapper = {}
        
        if feedback_column:
            for index, audio, text, score in zip(self.indices, self.audios, inputs, self.scores):
                self.mapper[index] = [audio, text, score]
        else:
        ...
        if num_examples != -1:
            ...
            self.scores = self.indices[:num_examples]
    ...
    def __getitem__(self, index):
        ...
        # return s1, s2, s3
        s4 = self.scores[index] if self.score else None
        return (s1, s2, s3, s4) if s4 is not None else (s1, s2, s3)
...
def main():
    ...
    if args.finetune_file is not None:
        finetune_data_files["finetune"] = args.finetune_file
        
    finetune_features = Features({
        'dataset': Value('string'),
        'location': Value('string'),
        'captions': Value('string'),
        'feedback': Value('float32'),
    })
    
    extension = args.train_file.split(".")[-1]
    raw_datasets = load_dataset(extension, data_files=data_files)
    finetune_datasets = load_dataset(extension, data_files=finetune_data_files, features=finetune_features)
    text_column, audio_column = args.text_column, args.audio_column, args.feedback_column
    ...
    with accelerator.main_process_first():
        ...
        finetune_dataset = Text2AudioDataset(raw_datasets["finetune"], prefix, text_column, audio_column, args.num_examples, feedback_column)
        ...
        accelerator.print("Num instances in train: {}, finetune:{}, validation: {}, test: {}".format(train_dataset.get_num_instances(), finetune_dataset.get_num_instances(), eval_dataset.get_num_instances(), test_dataset.get_num_instances()))
    ...
    finetune_dataloader = DataLoader(finetune_dataset, shuffle=True, batch_size=args.per_device_finetune_batch_size, collate_fn=finetune_dataset.collate_fn)
    ...
    train_dataloader, finetune_dataloader, eval_dataloader, test_dataloader = accelerator.prepare(
        train_dataloader, finetune_dataloader, eval_dataloader, test_dataloader
    )
    ...
    logger.info(f"  Num finetune_examples = {len(finetune_dataset)}")
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        total_pretrain_loss, total_finetune_loss, total_loss, total_val_loss = 0, 0, 0, 0
        for step, (pretrain_batch, finetune_batch) in enumerate(train_dataloader, finetune_dataloader):

            with accelerator.accumulate(model):
                device = model.device
                text, audios, _ = pretrain_batch
                finetune_text, finetune_audios, _, finetune_score = finetune_batch
                ...
                with torch.no_grad():
                    ...
                    # finetune data caculate
                    finetune_mel, _, _ = torch.tools.wav_to_fbank(finetune_audios, target_length, stft)
                    finetune_mel = finetune_mel.unsqeeze(1).to(device)
                    finetune_true_latent = unwrapped_vae.get_first_stage_encoding(unwrapped_vae.encode_first_stage(finetune_mel))
                    
                _, pretrain_loss = model(true_latent, text, validation_mode=False)
                pretrain_loss = args.beta * pretrain_loss
                total_pretrain_loss += pretrain_loss.detach().float()
                
                finetune_loss_df, _ = model(finetune_true_latent, finetune_text, validation_mode=False)
                finetune_score = torch.tensor(finetune_score).to(device)
                finetune_loss = finetune_score * finetune_loss_df
                finetune_loss = finetune_loss.mean()
                total_finetune_loss += finetune_loss.detach().float()
                
                loss = pretrain_loss + finetune_loss
        ...
        for step, batch in enumerate(eval_dataloader):
            with accelerator.accumulate(model) and torch.no_grad():
                ...
                _, val_loss = model(true_latent, text, validation_mode=True)
        ...
        if accelerator.is_main_process:    
            ...
            result["finetune_loss"] = round(total_finetune_loss.item()/len(finetune_dataloader), 4)
            result["loss"] = result["train_loss"] + result["finetune_loss"]
            result["val_loss"] = round(total_val_loss.item()/len(eval_dataloader), 4)

            wandb.log(result)

            result_string = "Epoch: {}, Loss Train: {}, Loss Finetune: {}, Val: {}\n".format(epoch, result["train_loss"], result["finetune_loss"], result["val_loss"])
